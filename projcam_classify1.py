...
class CleanerModel():
    ...
    def log_reg_model(self):
        ...
        # textted = get_json("cleanedexp_ted_10.json")
        # tokenizationted = nltk.word_tokenize(textted)[75:] #removing weird junk from the top of the andrew solomon talk
        # tokenizationpc = nltk.word_tokenize(textce)
        
        tedtokens = get_json("token_ted.json")
        pctokens = get_json("token_pc.json")
        ...
        n=50
        tedchunks = [tedtokens[i:i + n] for i in range(0, len(tedtokens), n)]
        pcchunks = [pctokens[i:i + n] for i in range(0, len(pctokens), n)]
        for t in tedchunks:
            #print(t)
            strt = ""
            for i in t:
